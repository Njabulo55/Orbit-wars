...
|------|-------------|--------------------------|--------------|
| `submission_v26plus.py` | **v26 + comet up-weight (COMET_VALUE_MULT 0.65→1.50)**. Identical strategy layer to v26 with comets prioritized over far neutrals. | _running, see STATUS.md_ | **Yes — lowest-risk upgrade vs v14.8** |
| `v26.py` | The Tamrazov+Ykhnkf hybrid with the aim-convergence fix. ~3400 lines of strategic features (snipe, swarm, gang-up, rescue, recapture, reinforce, crash-exploit, weakest-enemy targeting, total-war endgame). STATUS_after_v28.md describes this tier of agent as ~62% vs v14.8. | (baseline) | Fallback |
| `main.py` | **Clean rewrite (v4)** — 700 lines, exact-math aim solver (swept-pair verified, off-by-one corrected), per-planet timeline simulation for defense reservation. Easier to iterate on but missing v26's strategic features. | 12W/12L = 50% CI[31–69] | Experimental |

## How to submit

```bash
# Rename whichever candidate you want, then submit:
cp submission_v26plus.py main.py    # recommended
kaggle competitions submit orbit-wars -f main.py -m "v26 + comet up-weight"

# Or for the clean experimental agent:
