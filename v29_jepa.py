...
# === v22: distance-aware minimum fleet size ===
# Small fleets travel slowly: a 6-ship fleet at speed ~1.66 takes 30+ turns to cross 50 units.
# That's enough time for orbital prediction errors to compound, sending fleets off-board.
# Solution: scale minimum send size with travel distance, so distant targets get bigger fleets.

def v22_min_ships_for_distance(dist):
    """Return minimum recommended fleet size for a given travel distance.
    Tuning: at distance 10, allow 16 ships (~speed 2.6, ~4 turns, predictable).
    At distance 50, require 35 ships (~speed 4.2, ~12 turns, still manageable).
    At distance 80, require 60 ships (~speed 5.0, ~16 turns).
    """
    if dist <= 15:
        return 16
    elif dist <= 30:
...
EARLY_TURN_LIMIT = 40
# v19: rush phase — minimal proactive defense to force early expansion
EARLY_RUSH_TURN_LIMIT = 30          # turns < this use rush PDR
EARLY_RUSH_PDR = 0.10               # near-zero defense in rush phase
...
HOSTILE_TARGET_VALUE_MULT = 2.30   # v20c
...
HOSTILE_REINFORCE_HORIZON = 8       # ykhnkf patch
HOSTILE_REINFORCE_RATIO = 0.25      # ykhnkf patch
HOSTILE_REINFORCE_CAP = 15          # ykhnkf patch
...
FOLLOWUP_MIN_SHIPS = 12             # v22: was 8 — bigger followups
...
REAR_SEND_MIN_SHIPS = 18            # v22: was 10
REAR_MAX_TRAVEL_TURNS = 40

PARTIAL_SOURCE_MIN_SHIPS = 25       # v29-D: chunkier fleets
...
PROACTIVE_DEFENSE_RATIO = 0.40      # v19: bumped from 0.28 (sweep winner)
...
# v29-A: Comet anticipation
COMET_SPAWN_STEPS = (49, 149, 249, 349, 449)   # trigger one step before spawn
COMET_SPAWN_POSITIONS = [(75, 25), (25, 25), (75, 75), (25, 75)]   # quadrant spawn coords
COMET_ANTICIPATION_SHIPS_RATIO = 0.30   # fraction of source ships to send
COMET_ANTICIPATION_MIN_SHIPS = 20

# v29-B: Sweep instrumentation
SWEEP_PARK_LOOKAHEAD = 20       # turns ahead to look for orbiter sweep paths
SWEEP_PARK_MIN_SHIPS = 20       # minimum ships to park in sweep path
SWEEP_PARK_SCORE_MULT = 1.15    # bonus multiplier for sweep parking missions
OOB_SWEEP_RADIUS_MULT = 1.2     # radius multiplier for sweep detection

# v29 JEPA
JEPA_TOP_K_CANDIDATES = 60     # max candidates passed from JEPA screener to decoder
JEPA_MIN_VALUE_THRESHOLD = 0.1  # minimum JEPA value to consider
...
def aim_with_prediction(src, target, ships, initial_by_id, ang_vel, comets, comet_ids):
    """v26: convergence-guarded fixed-point intercept search.

    The original returned its last (non-converged) guess on failure, which sent
    ~60% of those fleets off-board. v26 instead falls back to search_safe_intercept
    (which is a brute-force search and usually finds something) and returns None
    if even that fails.

    Also bumped iteration limit 5->15 with damping to give iteration more chances
    to settle before falling back.
    """
    ...
    for it in range(15):  # v26: was 5
        ...
        # v26: damp toward the new estimate to break oscillation. Each iteration
        # moves halfway toward the new prediction.
        if it >= 5:
            tx = (tx + ntx) * 0.5
            ty = (ty + nty) * 0.5
        else:
    ...
    # v26: did NOT converge. Don't return a broken guess.
    # Try search_safe_intercept as a last resort — it does brute force and usually finds a valid aim.
    return search_safe_intercept(
        src, target, ships, initial_by_id, ang_vel, comets, comet_ids,
    )
...
# ============================================================
# Part 3: JEPA Architecture (v29)
# ============================================================

class JEPAStateEncoder:
    """Context encoder: compress board state into latent vector s_x (v29 JEPA)."""
    LATENT_DIM = 18

    def encode(self, world) -> list:
        total_ships = max(1, world.total_visible_ships)
        total_prod = max(1, world.total_production)
        my_ships_ratio = world.my_total / total_ships
        my_prod_ratio = world.my_prod / total_prod
        domination = (world.my_total - world.enemy_total) / max(1, world.my_total + world.enemy_total)
        prod_edge = (world.my_prod - world.enemy_prod) / max(1, world.my_prod + world.enemy_prod)

        my_fleets_ships = sum(int(f.ships) for f in world.fleets if f.owner == world.player)
        enemy_fleets_ships = sum(int(f.ships) for f in world.fleets if f.owner not in (-1, world.player))
        fleet_pressure = (my_fleets_ships - enemy_fleets_ships) / max(1, my_fleets_ships + enemy_fleets_ships)

        quad_my = [0.0, 0.0, 0.0, 0.0]
        quad_total = [1e-6, 1e-6, 1e-6, 1e-6]
        for p in world.planets:
            q = (1 if p.x >= 50 else 0) + (2 if p.y >= 50 else 0)
            w = float(max(1, p.production))
            quad_total[q] += w
            if p.owner == world.player:
                quad_my[q] += w
        quad_control = [quad_my[i] / quad_total[i] for i in range(4)]

        neutral_prod = sum(p.production for p in world.neutral_planets)
        neutral_opportunity = neutral_prod / total_prod

        uncaptured_comets = sum(1 for p in world.neutral_planets if p.id in world.comet_ids)
        comet_score = uncaptured_comets / max(1, len(world.comet_ids) + 1)

        threat_ships = sum(
            int(f.ships) for f in world.fleets
            if f.owner not in (-1, world.player)
            and any(dist(f.x, f.y, p.x, p.y) < 40 for p in world.my_planets)
        )
        threat_level = threat_ships / max(1, world.my_total)

        weakest_ratio = 0.0
        if world._weakest_enemy is not None:
            weakest_ratio = world._weakest_enemy_strength / max(1, world.my_total)
        ...
        return [
            ...
            world.remaining_steps / TOTAL_STEPS,
            len(world.my_planets) / max(1, len(world.planets)),
            weakest_ratio,
            float(world.is_four_player),
            float(world.is_total_war),
        ]


class JEPAPredictor:
    """JEPA predictor: predict future board value without full simulation."""

    def fast_garrison_estimate(self, target, turns, arrivals_by_planet, player):
        owner = target.owner
        garrison = float(target.ships)
        if owner != -1:
            garrison += target.production * turns
        events = arrivals_by_planet.get(target.id, [])
        by_turn = defaultdict(list)
        for eta, evt_owner, ships in events:
            eta_int = max(1, int(math.ceil(eta)))
            if eta_int <= turns:
                by_turn[eta_int].append((eta_int, evt_owner, int(ships)))
        for t in sorted(by_turn):
            owner, garrison = resolve_arrival_event(owner, garrison, by_turn[t])
        return owner, max(0.0, garrison)

    def predict_action_value(self, s_x, src, target, ships, turns, world):
        target_owner, target_garrison = self.fast_garrison_estimate(
            target, turns, world.arrivals_by_planet, world.player
        )
        enemy_contention = sum(
            int(s) for eta, o, s in world.arrivals_by_planet.get(target.id, [])
            if o not in (-1, world.player) and abs(int(math.ceil(eta)) - turns) <= 2
        )
        effective_ships = ships - enemy_contention
        if effective_ships <= 0:
            return -1.0, None
        if enemy_contention > 0 and enemy_contention >= effective_ships:
            return -1.0, None
        turns_remaining = max(1, world.remaining_steps - turns)
        if target_owner == world.player:
            return effective_ships * 0.2, None
        if effective_ships <= target_garrison:
            return 0.0, None
        production_gain = target.production * turns_remaining
        ship_cost = ships * 0.6
        q = (1 if target.x >= 50 else 0) + (2 if target.y >= 50 else 0)
        quad_idx = 5 + q
        quad_bonus = (1.0 - s_x[quad_idx]) * target.production * 8.0
        value_est = production_gain + quad_bonus - ship_cost
        if target_owner not in (-1, world.player):
            value_est *= 1.5
            if target_owner == world._weakest_enemy:
                value_est *= 1.4
        if value_est <= 0:
            return 0.0, None
        s_y = list(s_x)
        prod_delta = target.production / max(1, world.total_production)
        s_y[1] = min(1.0, s_y[1] + prod_delta)
        s_y[2] = min(1.0, s_y[2] + 2.0 * prod_delta)
        s_y[quad_idx] = min(1.0, s_y[quad_idx] + 0.1)
        return value_est, s_y

    def rank_candidates(self, candidates, world):
        return sorted(candidates, key=lambda item: -item[0])


class JEPADecoder:
    """Translates JEPA latent trajectories back into explicit API commands."""

    def decode_action(self, src, target, src_available, world, planned_commitments, modes, policy, mission="capture"):
        ...
        plan = settle_plan(src, target, src_available, send_guess, world, planned_commitments, modes, policy, mission=mission)
        return plan


class WorldModel:
    _oob_log = defaultdict(list)  # v29-B: OOB tracking

    def log_oob_event(self, turn, fleet_owner, ships, reason):
        """v29-B: Log a fleet that disappeared (OOB) for diagnostics."""
        WorldModel._oob_log[turn].append({
            "owner": fleet_owner, "ships": ships, "reason": reason
        })
    ...
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        # v29 JEPA: build latent state representation once per turn
        _jepa_enc = JEPAStateEncoder()
        self.jepa_latent = _jepa_enc.encode(self)
        self.jepa_predictor = JEPAPredictor()
        self.jepa_decoder = JEPADecoder()
    ...
    def probe_ship_candidates(self, src_id, target_id, source_cap, hints=()):
        ...
        values = set(range(PARTIAL_SOURCE_MIN_SHIPS, min(PARTIAL_SOURCE_MIN_SHIPS + 8, source_cap) + 1))  # v23: no tiny probes
        values.update({
            source_cap,
            max(PARTIAL_SOURCE_MIN_SHIPS, source_cap // 2),
            max(PARTIAL_SOURCE_MIN_SHIPS, source_cap // 3),
            min(source_cap, PARTIAL_SOURCE_MIN_SHIPS),
            # v23: when responding to a target, ensure send is at least PARTIAL_SOURCE_MIN_SHIPS
            min(source_cap, max(PARTIAL_SOURCE_MIN_SHIPS, target_ships + 1)),
            min(source_cap, max(PARTIAL_SOURCE_MIN_SHIPS, target_ships + 2)),
            min(source_cap, max(PARTIAL_SOURCE_MIN_SHIPS, target_ships + 4)),
            min(source_cap, max(PARTIAL_SOURCE_MIN_SHIPS, target_ships + 8)),
        })

        for hint in normalized_hints:
            base = max(PARTIAL_SOURCE_MIN_SHIPS, min(source_cap, hint))
            for delta in (-2, -1, 0, 1, 2):
                candidate = base + delta
                if PARTIAL_SOURCE_MIN_SHIPS <= candidate <= source_cap:  # v23: no tiny candidates
...
def inbound_enemy_ships_at(world, target_id, arrival_turn, tolerance=2):
    """Return total enemy ships arriving near arrival_turn at target_id (v29-C)."""
    return sum(
        int(ships)
        for eta, owner, ships in world.arrivals_by_planet.get(target_id, [])
        if owner not in (-1, world.player)
        and abs(int(math.ceil(eta)) - arrival_turn) <= tolerance
    )
    ...
    for planet in world.my_planets:
        ...
        for enemy in nearest_sources_to_target(planet, world.enemy_planets, PROACTIVE_ENEMY_TOP_K):
            ...
            if enemy_eta > PROACTIVE_DEFENSE_HORIZON:
                continue
            # v19: rush phase uses near-zero PDR; otherwise use the bumped value
            _pdr = EARLY_RUSH_PDR if world.step < EARLY_RUSH_TURN_LIMIT else PROACTIVE_DEFENSE_RATIO
            proactive_keep = max(proactive_keep, int(enemy.ships * _pdr))
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    if target.owner == -1:
        ...
        # ykhnkf HOSTILE_REINFORCE patch: anticipate enemy reinforcement ships.
        # Tamrazov stores enemies as flat list in world.enemy_planets (no opp_planets dict).
        try:
            reinforce_est = 0
            for ep in world.enemy_planets:
                if ep.owner != target.owner or ep.id == target.id:
                    continue
    ...
    for _ in range(max_iter):
        ...
        if desired == actual_send:
            ...
            if mission == "rescue" and turns > eval_turn:
                return None
            # v29-C: if enemy inbound >= our send they annihilate us
            _v29c_inbound = inbound_enemy_ships_at(world, target.id, turns)
            if _v29c_inbound >= actual_send:
                return None
...
def build_comet_anticipation_missions(world, policy, planned_commitments, modes):
    """v29-A: Pre-position ships near comet spawn points one step before spawn."""
    if world.step not in COMET_SPAWN_STEPS:
        return []
    result = []
    for cx, cy in COMET_SPAWN_POSITIONS:
        ...
        for src in world.my_planets:
            ...
            send = max(COMET_ANTICIPATION_MIN_SHIPS, int(src.ships * COMET_ANTICIPATION_SHIPS_RATIO))
            if budget < send:
                continue
            d = dist(src.x, src.y, cx, cy)
            if d < best_d:
                best_d = d
                best_src = src
        if best_src is None:
            continue
        send = max(COMET_ANTICIPATION_MIN_SHIPS, int(best_src.ships * COMET_ANTICIPATION_SHIPS_RATIO))
        send = min(send, policy["attack_budget"].get(best_src.id, 0))
        if send < COMET_ANTICIPATION_MIN_SHIPS:
            continue
        angle = math.atan2(cy - best_src.y, cx - best_src.x)
        result.append((best_src.id, angle, send))
    return result


def build_sweep_parking_missions(world, policy, planned_commitments, modes):
    """v29-B: Park fleets in the sweep paths of orbiting enemy planets."""
    result = []
    for enemy in world.enemy_planets:
        if is_static_planet(enemy):
            continue
        if int(enemy.ships) < 30:
            continue
        for t in [5, 10, 15, 20]:
            fx, fy = predict_planet_position(
                enemy, world.initial_by_id, world.ang_vel, t
            )
            sweep_r = enemy.radius * OOB_SWEEP_RADIUS_MULT
            for src in world.my_planets:
                budget = policy["attack_budget"].get(src.id, 0)
                if budget < SWEEP_PARK_MIN_SHIPS:
                    continue
                d = dist(src.x, src.y, fx, fy)
                spd = fleet_speed(budget // 2)
                travel_turns = max(1, int(math.ceil(d / spd)))
                if abs(travel_turns - t) > 3:
                    continue
                angle = math.atan2(fy - src.y, fx - src.x)
                send = min(budget, max(SWEEP_PARK_MIN_SHIPS, budget // 2))
                result.append((src.id, angle, send))
    return result
...
def plan_moves(world, deadline=None):
    ...
    # === v14.8 dynamic hyperparameters + Memex ===
    domination = modes["domination"]
    # Memex write
    for fleet in world.fleets:
        if fleet.owner != world.player and fleet.owner != -1:
            MEMEX_ARCHIVE[fleet.owner].append({'turn': world.step, 'ships': int(fleet.ships), 'x': fleet.x, 'y': fleet.y})
            if len(MEMEX_ARCHIVE[fleet.owner]) > MEMEX_MAX_RAW:
                MEMEX_ARCHIVE[fleet.owner] = MEMEX_ARCHIVE[fleet.owner][-MEMEX_MAX_RAW:]
    # Memex manage
    for enemy in getattr(world, 'enemy_ids', []):
        recent = [e for e in MEMEX_ARCHIVE[enemy] if world.step - e['turn'] <= 20]
        if recent:
            total = sum(e['ships'] for e in recent)
            aggression = min(1.5, total / max(1, world.owner_strength.get(enemy,1)*1.5))
            MEMEX_SUMMARIES[enemy] = {'aggression': aggression, 'last_seen': world.step}
    # reinforce-to-hold
    for planet in world.my_planets:
        if planet.ships > planet.production * 5: continue
        incoming = sum(f.ships for f in world.fleets if f.owner != world.player and ((f.x-planet.x)**2+(f.y-planet.y)**2)**0.5 / max(0.1, 1+f.ships*0.005) <= 1.5)
        predicted = planet.ships + planet.production - incoming
        if 0 < predicted < planet.production * 3:
            donors = [p for p in world.my_planets if p.id != planet.id and p.ships > p.production*4]
            if donors:
                donor = min(donors, key=lambda p: (p.x-planet.x)**2+(p.y-planet.y)**2)
                send = min(int(donor.ships*0.25), int(planet.production*3 - predicted))
                if send >= 5:
                    aim = world.plan_shot(donor.id, planet.id, send)
                    if aim: 
                        # early moves list may not exist yet, skip for now
                        pass
    ...
    # v29-A: comet anticipation pre-launch
    _comet_premoves = build_comet_anticipation_missions(world, policy, planned_commitments, modes)
    for _cm_src_id, _cm_angle, _cm_send in _comet_premoves:
        append_move(_cm_src_id, _cm_angle, _cm_send)

    # v29-B: sweep parking pre-launch
    _sweep_premoves = build_sweep_parking_missions(world, policy, planned_commitments, modes)
    for _sw_src_id, _sw_angle, _sw_send in _sweep_premoves:
        append_move(_sw_src_id, _sw_angle, _sw_send)

    # === JEPA CANDIDATE SCREENING (v29) ===
    _jepa_candidates = []
    for _jsrc in world.my_planets:
        _jsrc_available = source_attack_left(_jsrc.id)
        if _jsrc_available <= 0:
            continue
        for _jtarget in world.planets:
            if _jtarget.id == _jsrc.id or _jtarget.owner == world.player:
                continue
            _jships_hint = max(PARTIAL_SOURCE_MIN_SHIPS, _jsrc_available // 2)
            _jd = dist(_jsrc.x, _jsrc.y, _jtarget.x, _jtarget.y)
            _jspd = fleet_speed(_jsrc.ships // 2) * 0.8
            _jturns_hint = max(1, int(_jd / max(0.1, _jspd)))
            _jval, _js_y = world.jepa_predictor.predict_action_value(
                world.jepa_latent, _jsrc, _jtarget, _jships_hint, _jturns_hint, world
            )
            if _jval >= JEPA_MIN_VALUE_THRESHOLD:
                _jepa_candidates.append((_jval, _jsrc, _jtarget, _jships_hint))

    _jepa_candidates = world.jepa_predictor.rank_candidates(_jepa_candidates, world)[:JEPA_TOP_K_CANDIDATES]
    _jepa_decoded_count = 0
    for _jval, _jsrc, _jtarget, _jships_hint in _jepa_candidates:
        if expired():
            break
        _jsrc_available = source_attack_left(_jsrc.id)
        if _jsrc_available < PARTIAL_SOURCE_MIN_SHIPS:
            continue
        _jplan = world.jepa_decoder.decode_action(
            _jsrc, _jtarget, _jsrc_available, world, planned_commitments, modes, policy
        )
        if _jplan is None:
            continue
        _jangle, _jturns, _, _jneed, _jsend = _jplan
        if _jsend < _jneed or _jneed <= 0:
            continue
        if not candidate_time_valid(_jtarget, _jturns, world,
                                    VERY_LATE_CAPTURE_BUFFER if world.is_very_late else LATE_CAPTURE_BUFFER):
            continue
        _jvalue = target_value(_jtarget, _jturns, "capture", world, modes, policy)
        if _jvalue <= 0:
            continue
        _jscore = apply_score_modifiers(
            _jvalue / (_jsend + _jturns * ATTACK_COST_TURN_WEIGHT + 1.0),
            _jtarget, "capture", world,
        )
        missions.append(Mission(
            kind="single", score=_jscore, target_id=_jtarget.id,
            turns=_jturns, options=[ShotOption(
                score=_jscore, src_id=_jsrc.id, target_id=_jtarget.id,
                angle=_jangle, turns=_jturns, needed=_jneed, send_cap=_jsend,
                mission="capture",
            )],
        ))
        _jepa_decoded_count += 1

    # === MAIN CANDIDATE LOOP (fallback if JEPA screening finds fewer than 10) ===
    if _jepa_decoded_count < 10:
        for src in world.my_planets:
            if expired():
            ...
            for target in world.planets:
                ...
                # v29-C: skip if enemy inbound >= our send
                enemy_inbound = inbound_enemy_ships_at(world, target.id, rough_turns)
                if enemy_inbound >= partial_send_cap or enemy_inbound >= global_needed:
                    continue
...
The cell below packages `submission.py` into a tar (Kaggle competitions accept either a single `.py` or a tar.gz). Submit through the *Submit Predictions* button after this notebook is committed.

```bash
CLI alternative:
kaggle competitions submit orbit-wars -f submission.py -m "Tamrazov×Ykhnkf hybrid"
```
